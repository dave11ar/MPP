...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    override fun deposit(index: Int, amount: Long): Long { // First, validate method per-conditions
        ...
        while (true) {
            ...
            val updatedAmount = account.amount + amount
            check(updatedAmount <= MAX_AMOUNT) { "Overflow" }
            if (accounts[index].compareAndSet(account, Account(updatedAmount))) return updatedAmount
        }
    }
    ...
    override fun withdraw(index: Int, amount: Long): Long {
        ...
        while (true) {
            ...
            if (account.invokeOperation()) continue
            val updatedAmount = account.amount - amount
            check(updatedAmount >= 0) { "Underflow" }
            if (accounts[index].compareAndSet(account, Account(updatedAmount))) return updatedAmount
        }
    }
